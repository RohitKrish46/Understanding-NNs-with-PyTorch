...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # create optim groups. Any parameter that is 2D will be weight decayed, else no
        # ie all weight tensors in matmul, embeddings, all biases and layernorms dont
...
total_batch_size = 524288 # ~0.5 M, desired number of tokens to process
B = 8 # mini batch size
...
def get_lr(it):
    ...
    if it > max_steps:
        return min_lr
    # if in between, use cosine decay
...
# create log dir for checkpoints logging (train loss, val loss, hellaswag accuracies)
log_dir = "log"
...
for step in range(max_steps):
    ...
    t1 = time.time()
    dt = (t1 - t0)  # time diff in ms
    tokens_processed = train_loader.B * train_loader.T * grad_accum_steps
    tokens_per_sec = tokens_processed / dt
    print(f"step {step:4d} | loss: {loss_accum.item():.4f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {dt* 1000:.2f}ms | tok/sec: {tokens_per_sec:.2f}")
    with open(log_file, "a") as f:
